...
RUN curl -fsSLO "https://nodejs.org/dist/v${NODE_VERSION}/node-v${NODE_VERSION}-linux-arm64.tar.xz" \
    && mkdir -p /opt/node \
    && tar -xJf "node-v${NODE_VERSION}-linux-arm64.tar.xz" --strip-components=1 -C /opt/node \
...
RUN scripts/verify-linux-deb.sh dist/dsh-desktop-linux-arm64.deb arm64
RUN apt-get -o Acquire::Check-Valid-Until=false update \
    && DEBIAN_FRONTEND=noninteractive apt-get install -y \
      ./dist/dsh-desktop-linux-arm64.deb \
...
COPY --from=build /workspace/dist/dsh-desktop-linux-arm64.deb /dsh-desktop-linux-arm64.deb
